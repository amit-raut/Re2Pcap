...
def parse_Response(res_file):
    try:
        ...
        return {
            "error": "Failed to Parse Given HTTP Raw Response. Please Verify your Input and Try Again."
        }
    return response
...
def parse_Request(req_file):
    try:
        ...
        return {
            "error": "Failed to Parse Given HTTP Raw Request. Please Verify your Input and Try Again."
        }
...
def createPcap():
    ...
    inputResponse = request.form["inputResponse"]
    resultFileName = (
        request.form["pcapFileName"] + ".pcap"
        if request.form["pcapFileName"]
        else "Re2Pcap-Result.pcap"
    )
    ...
    try:
        if len(command) > 5:
            ...
            if not childProc.returncode:
                ...
                return jsonify(
                    error="Something went Wrong :( Please Check the Input/ Error log and Try Again ...."
                )
        else:
            return jsonify(
                error="Something went Wrong :( Please Check the Input/ Error log and Try Again ...."
...
if __name__ == "__main__":
    ...
    app.run(
        host=os.getenv("IP", host_ip), port=int(os.getenv("PORT", 5000)), debug=True
    )
